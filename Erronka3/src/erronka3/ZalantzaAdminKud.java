...
public class ZalantzaAdminKud extends MenuAdminNagusia {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ZalantzaAdminKud frame = new ZalantzaAdminKud();
	...
	public ZalantzaAdminKud() {
		...
		JButton ezabatuZalantza = new JButton("Ezabatu");
		ezabatuZalantza.setBounds(700, 30, 100, 20);
		ezabatuZalantza.setBackground(new Color(255, 128, 128));
		contentPane.add(ezabatuZalantza);
		...
		ezabatuZalantza.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				...
				if (kontrola.erregistroaKomprobatu(zalantzak, row)) {
					...
					try {
						...
						if (zalantza != null) {
							if (kontrola.ezabatuZalantza(idZalantza)) {
								JOptionPane.showMessageDialog(null, "Zalantza ongi ezabatuta.");
								TaulaKargatu(zalantza.zalantzakBistaratu());
							}
						} else {
							JOptionPane.showMessageDialog(null, "Ezin izan da zalantza ezabatu.");
						}
					} catch (HeadlessException e1) {
						e1.printStackTrace();
					}
				} else {
					JOptionPane.showMessageDialog(null, "Aukeratu zalantza bat ezabatzeko.");
				}
			}
		});
